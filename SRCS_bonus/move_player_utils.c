...
/*   Updated: 2024/12/07 00:12:22 by nnelo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	animation_player(int keycode, t_data *data, int new_x, int new_y)
{
	...
	if (keycode == 'a')
	{
		...
		data->player = mlx_xpm_file_to_image(data->mlx_ptr, "textures/purple_turtle_90.xpm", &width, &height);
	}
	if (keycode == 'd')
	{
		mlx_destroy_image(data->mlx_ptr, data->player);
		data->player = mlx_xpm_file_to_image(data->mlx_ptr, "textures/purple_turtle.xpm", &width, &height);
	}
	data->player_x = new_x;
	data->player_y = new_y;
}